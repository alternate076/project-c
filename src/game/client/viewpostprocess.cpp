...
void DoEnginePostProcessing( int x, int y, int w, int h, bool bFlashlightIsOn, bool bPostVGui )
{
	...
	static IMaterial *pMat = materials->FindMaterial("effects/postproc_vignette", TEXTURE_GROUP_OTHER);
	if (pMat)
	{
	...
	static IMaterial *pMat2 = materials->FindMaterial( "effects/postproc_smartnoise", TEXTURE_GROUP_OTHER );
	if (pMat2)
	{
