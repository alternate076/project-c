...
{
	return !(m_pDrawBrushModels && !m_pDrawBrushModels->GetInt());
}
...
void CViewRender::UpdateCascadedShadow(const CViewSetup& view)
{
	...
	cascadedShadowView.zFar = cascadedShadowView.zFarViewmodel = 8162.0f;
	cascadedShadowView.zNear = cascadedShadowView.zNearViewmodel = -4096.0f;
	...
	for (int i = 0; i != 12; ++i)
	{
		...
		else
		{
			...
			if (iClipped[pPoints[0]] == 1)
			{
				...
				Assert(fabsf(vNormal.Dot(vSplitPoints[pPoints[0]][pPoints[1]]) - fPlaneDist) < 0.01f);
